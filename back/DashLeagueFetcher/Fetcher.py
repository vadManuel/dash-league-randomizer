...
class Fetcher:
	...
	def __get_data_by_season_cycle__(self, url=None):
		data = dict()

		for cycle in range(1, self.n_cycles + 1):
			# building url for current_season and enumerated cycle
			_url = url.replace('<season>', str(self.current_season)).replace('<cycle>', str(cycle))
	...
	def fetch_data(self):
		...
		data_by_tiers_season_cycle = self.__get_data_by_season_cycle__(url=url_tiers_season_cycle)
		...
		data_by_matchups_season_cycle = self.__get_data_by_season_cycle__(url=url_matchups_season_cycle)
